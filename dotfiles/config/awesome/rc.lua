...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "j",
        function ()
            awful.client.focus.byidx( 1)
            -- naughty.notify({title = gears.filesystem.get_dir("config")})
        end,
    ...
    awful.key({ modkey }, "Print",
      ...
      {description = "make a screenshot", group = "my"}),

    awful.key({ "Shift" }, "Print",
      function ()
        awful.util.spawn_with_shell("sleep 0.5 && scrot -s -e 'mv $f ~/pics/shots/ 2>/dev/null'")
        -- naughty.notify({title="scrot", text="took a screenshot"})
      end,
